...
def create_pii_prompt(text: str, is_finetuned: bool = False) -> str:
    ...
    if is_finetuned:
        # Finetuned model - use exact training format for best performance
        ...
        # Base model - simpler, less specific prompt that won't catch everything
        system = "You are a helpful assistant."
        instruction = f"Replace all personally identifiable information (PII) and protected health information (PHI) in the following document from the general domain:\n\n{text}"
...
async def mask_pii(request: PIIRequest):
    ...
    try:
        # Generate with base model (potentially less effective due to generic prompt)
        base_prompt = create_pii_prompt(request.text, is_finetuned=False)
        base_output, base_time = generate_with_gguf(
            base_model, 
            base_prompt,
            request.max_tokens,
            request.temperature * 2  # Slightly higher temp for base model
        )
        
        # Generate with finetuned model (should be more effective)
